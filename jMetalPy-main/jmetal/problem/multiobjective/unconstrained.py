...
class OneZeroMax(BinaryProblem):
    ...
    def __init__(self, number_of_bits: int = 1):
        super(OneZeroMax, self).__init__()
        self.number_of_bits = number_of_bits
        # print("bits", self.number_of_bits)
    ...
    def evaluate(self, solution: BinarySolution) -> BinarySolution:
        ...
        counter_of_zeroes = 0
        # ture是0 false是1
        for var in range(solution.number_of_variables):
        ...
        if x[0] == True:
            ...
            tag = 1
        solution.objectives[2] = tag * tag - 2 * tag + 1
        print("lenconstraint", len(solution.constraints))
        solution.constraints[0] = self.calculate_constraint_a(True)
        solution.constraints[1] = self.calculate_constraint_b(True)
        return solution

    def calculate_constraint_a(self, x):
        if x:
            return 0.0  # 符合约束条件
        else:
            return -100  # 不符合约束条件，返回违反程度

    def calculate_constraint_b(self, x):
        if x:
            return 0.0  # 符合约束条件
        else:
            return -100  # 不符合约束条件，返回违反程度
    ...
    def create_solution(self) -> BinarySolution:
        ...
        for var in range(self.number_of_variables()):
            new_solution.variables[var] = [
...
class MixedIntegerFloatProblem(Problem):
    ...
    def create_solution(self) -> CompositeSolution:
        ...
        integer_solution.variables = [
            random.uniform(self.int_lower_bound[i], self.int_upper_bound[i]) for i in range(len(self.int_lower_bound))
        ]
