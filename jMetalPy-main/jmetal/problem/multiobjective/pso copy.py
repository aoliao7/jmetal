...
class PSO:
    ...
    def greedy_solution(self):
        ...
        GS = list(self.GS)
        allocate_fail = self.allocate(ids, GS)
        for i in allocate_fail:
            print(i + " 未分配")

        # 2.顺丰给16、18机位(目前数据没货运)
        ids = []
        FSF = self.FSF.sort_values(by=["sta"], ascending=True)
        for index, f in FSF.iterrows():
            ids.append(index)
        total_ids.update(ids)
        G = [self.G_number[16], self.G_number[18]]
        allocate_fail = self.allocate(ids, G)
        for i in allocate_fail:
            print(i + " 未分配")

        # 3.货机(目前数据没货运)
        ids = []
        FC = self.FC.sort_values(by=["sta"], ascending=True)
        for index, f in FC.iterrows():
            ids.append(index)
        total_ids.update(ids)
        GC = list(self.GC)
        allocate_fail = self.allocate(ids, GC)
        for i in allocate_fail:
            print(i + " 未分配")
        ...
        total_ids.update(ids)
        GI = list(self.GI)
        allocate_fail = self.allocate(ids, GI)
        for i in allocate_fail:
            print(i + " 未分配")
        ...
        allocate_fail = self.allocate(ids, GDB)
        for i in allocate_fail:
            print(i + " 未分配")
        ...
        # 国内客机位
        GB = list(self.GB)
        allocate_fail = self.allocate(ids, GB)
        GF = list(self.GU - self.GI - self.GB)
        allocate_fail = self.allocate(allocate_fail, GF)
        GC = list(self.GC)
        allocate_fail = self.allocate(allocate_fail, GC)
        for i in allocate_fail:
            print(i + " 未分配")
    ...
    def evaluate_total(self):
        ...
        print("原始value：", [float("{:.4f}".format(i)) for i in value])

        mean = np.mean(value)
        var = np.var(value)
        for i in range(len(value)):
            value[i] = (value[i] - mean) / var
        print("归一化value：", [float("{:.4f}".format(i)) for i in value])
