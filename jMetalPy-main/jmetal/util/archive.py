...
class NonDominatedSolutionsArchive(Archive[S]):
    ...
    def add(self, solution: S) -> bool:
        ...
        if len(self.solution_list) == 0:
            # flag = 0
            # for x in range(solution.number_of_constraints):
            #     if solution.constraints[x] != 0:
            #         flag = 1
            #         break
            # if flag == 0:
            ...
            # return False
            sum = 0
            for op in range(solution.number_of_constraints):
                sum = sum + solution.constraints[op]
            if sum == 0:
                self.solution_list.append(solution)
                # print("solution_list0", solution.objectives)
                # print("solution_list00", solution.constraints)
                matrix = np.array(solution.variables).reshape(60, 144)
                flights_per_slot = np.sum(matrix, axis=1)
                # print("每个机位停飞机的数量00:", flights_per_slot)
            return True
        else:
            number_of_deleted_solutions = 0

            # New copy of list and enumerate
            # 1要1 -1 要-1  0 两个都要  -2 都不要
            # enumerate(list))复制一份，防止删除操作改变顺序
            for index, current_solution in enumerate(list(self.solution_list)):
                ...
                # print("is_dominated_flag", is_dominated_flag)
                if is_dominated_flag == 1:
                    ...
                    if (
                        ...
                        and solution.variables != current_solution.variables
                    ):
                        # is_contained = True
                        sum = 0
                        for op in range(solution.number_of_constraints):
                        ...
                        if sum == 0:
                            self.solution_list.append(solution)
                            # print("solution_list1", solution.objectives)
                            # print("solution_list11", solution.constraints)
                            return True
                    elif solution.objectives[0] < current_solution.objectives[0]:
                        sum = 0
                        for op in range(solution.number_of_constraints):
                            sum = sum + solution.constraints[op]
                        if sum == 0:
                            self.solution_list.append(solution)
                            # print("solution_list2", solution.objectives)
                            # print("solution_list22", solution.constraints)
                            return True
                    else:
                        return True
        ...
        if not is_dominated and not is_contained:
            ...
            if sum == 0:
                ...
                matrix = np.array(solution.variables).reshape(60, 144)
                flights_per_slot = np.sum(matrix, axis=1)
                # print("每个机位停飞机的数量33:", flights_per_slot)
                # print("solution_list3", solution.objectives)
                # print("solution_list33", solution.constraints)
                return True
        return False
